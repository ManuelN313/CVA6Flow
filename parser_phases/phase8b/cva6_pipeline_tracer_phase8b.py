...
class InstructionRecord:
    ...
    # boundary (8B in FETCH_WIDTH=64). Determined by PC + size:
    # wraps_line ↔ (pc & 7) == 6 AND not is_compressed. Equivalent
    # to the realigner asserting serving_unaligned_o at the cycle it
    # outputs this instruction. Captured on every record (committed
    # AND flushed) since the unaligned bookkeeping happens in the
    # realigner regardless of whether the instruction reaches commit.
...
def match_records_to_events(records, events):
    ...
    vaddr_word == (pc & ~0x3) with maximum fe2_cycle <= rec.fe_cycle.
    For aligned instructions this is the only fetch. RVC pair sharing
    falls out naturally: both halves of a pair have close fe_cycles
    and select the same event. Different loop iterations have
    well-separated fe_cycles and bind to their own iteration's event.

    Phase 8b (wraps_line records only): also bind if1_hi / if2_hi for
    the SECOND fetch, looking up the event whose vaddr_word ==
    ((pc + 2) & ~0x3) with the same fe2_cycle ordering rule. (For a
    32-bit instr at PC = A+6, the upper-half word lives at A+8.)
...
class PipelineTracker:
    ...
    def _compute_wraps_line(pc, is_compressed):
        ...
        (FW=64 → 8B blocks). Set on every record that has a valid PC
        and is a 32-bit instruction at offset 6 in its block."""
        if is_compressed or pc is None:
            return False
        try:
            return (int(pc, 16) & 0x7) == 0x6
        except (TypeError, ValueError):
            return False
...
def stream_and_extract(f, matches, args, n_wb_ports, n_commit_ports):
    ...
    print(f"Phase 8b: wraps_line records = {n_wraps} total "
