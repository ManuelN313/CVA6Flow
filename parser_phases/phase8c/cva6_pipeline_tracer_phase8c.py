...
def stream_and_extract(f, matches, args, n_wb_ports, n_commit_ports):
    ...
    if 0 < detected_nr_sb < NR_SB:
        print(f"Scoreboard depth: detected {detected_nr_sb} slots in VCD "
    ...
    if MEMQ_BP_AVAILABLE:
        print("Phase 7a: mem_q[*].sbe.bp.cf resolved. Using authoritative "
              "reads at writeback to correct the pre-edge decoded_instr_i "
              "bp.cf misattribution for back-to-back issues",
              file=sys.stderr)
    else:
        print(f"WARNING: Phase 7a. mem_q[*].sbe.bp.cf not resolved "
    ...
    print(f"Phase 8c: branch bubbles. "
    ...
    print(f"Phase 8c diag: {bubble_diag['bp_mispredict_total']} records "
...
VERIFIED_CONFIG_FIELDS = frozenset({
    ...
    "NrScoreboardEntries",
    "TRANS_ID_BITS",
    # Tier 2, structurally verified by trace success.
    "NrCommitPorts",
    "NrWbPorts",
    "FETCH_WIDTH",
    "INSTR_PER_FETCH",
...
def main():
    ...
    if cp_ts is not None and cp_ts >= 1000:  # >= 1 ns, plausible cycle
        ...
        print(f"Clock period: {period_disp} ({freq_disp}). "
              f"VCD timescale: {ts_unit}", file=sys.stderr)
        ...
        # its hardcoded clock.
        print(f"Clock period: detected {cp_ts} VCD ticks (implausibly "
