...
int main(int argc, char **argv) {
  ...
  #ifndef START_TRACE_CYCLE
  #define START_TRACE_CYCLE 0
  #endif
  uint64_t start = START_TRACE_CYCLE;

  #ifndef END_TRACE_CYCLE
  #define END_TRACE_CYCLE 0xFFFFFFFFFFFFFFFFULL 
  #endif
  uint64_t end_trace = END_TRACE_CYCLE;
#endif
  char ** htif_argv = NULL;
...
done_processing:
  ...
  for (int i = 0; i < 10; i++) {
    ...
    top->eval();
#if VM_TRACE
    if ((vcdfile || fst_fname) && main_time >= start && main_time <= end_trace)
